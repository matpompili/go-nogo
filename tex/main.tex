...
%	]{classicthesis}
\usepackage{geometry}
\geometry{lmargin=2.5cm,rmargin=2.5cm,bmargin=2.5cm,tmargin=2.5cm}
% ACRONYMS
...
The nodeOS controllers will communicate with each other using a direct classical link (an optical fibre, for example), while in the future implementations they would communicate on the Internet. The communication with the Physical link will use a protocol currently in development \todo{Check CQC}(\ac{CQC}) that will later be used on a metropolitan scale in the Demonstrator project.
